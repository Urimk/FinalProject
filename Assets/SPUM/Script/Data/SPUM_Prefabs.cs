...
public class SPUM_Prefabs : MonoBehaviour
{
    ...
    public List<AnimationClip> CHARGE_List = new();
    public List<AnimationClip> DASH_List = new();
        ...
        foreach (var kvp in groupedClips)
        {
            ...
            switch (stateType)
            {
                ...
                case "DEATH":
                    ...
                    break;
                case "CHARGE":
                CHARGE_List.AddRange(
                        orderedClips
                        .Where(clip => clip.ClipPath.Contains("ChargeDash"))  // Check if the path contains "ChargeDash"
                        .Select(clip => LoadAnimationClip(clip.ClipPath))      // Convert SpumAnimationClip to AnimationClip
                    );                    
                    break;
                case "DASH":
                CHARGE_List.AddRange(
                        orderedClips
                        .Where(clip => clip.ClipPath.Contains("Dash"))  // Check if the path contains "ChargeDash"
                        .Select(clip => LoadAnimationClip(clip.ClipPath))      // Convert SpumAnimationClip to AnimationClip
                    );                    
                    break;
                case "OTHER":
